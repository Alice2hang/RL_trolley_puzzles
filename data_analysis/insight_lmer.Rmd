...
```{r simr}
library(simr)
model_ext <- extend(model, along="userid", n=200)
sim_treat <- powerSim(model_ext, nsim=100, test = fcompare(score_dif~trial))
sim_treat
p_curve_treat <- powerCurve(model_ext, test=fcompare(score_dif~trial), along="userid")
plot(p_curve_treat)
```
